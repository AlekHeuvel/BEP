...
theta_slider =  Slider(ax_theta, '$θ_{ext}$ (radians)', -pt.pi, pt.pi, valinit=0)
E_ext_slider =  Slider(ax_E_ext, 'E_ext', 0, 5, valinit=0)
h_slider =      Slider(ax_h, 'h', 0, 2, valinit=1)
kappa_slider =  Slider(ax_kappa, '$κ$', 0, 2, valinit=1)
d_slider =      Slider(ax_d, 'd', -2, 2, valinit=1) # Negative d means dipole points away from the electric field
...
def update(val):
    ...
    # ax.plot_surface(X1, X2, wf.psi_sq_relative_normalized(X1, X2, a1, a2, b), cmap='viridis')
    ax.set_xlabel(r'x1 ($a_0$)') # In units of bohr radius = 4 pi epsilon_0 h_bar^2 / (m_e e+^2), = 5.2917e-11 m
    ax.set_ylabel(r'x2 ($a_0$)') # In units of bohr radius = 4 pi epsilon_0 h_bar^2 / (m_e e+^2), = 5.2917e-11 m
